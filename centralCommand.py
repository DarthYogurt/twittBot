...
def get_twitter_friend_list(list, cursor):
    ...
    :return:
        ...
        '''
    if cursor > 1:
        url = BASE_URL + 'friends/list.json?count=200&cursor=' + str(cursor)
    elif cursor == 0:
        return list
    elif cursor == 1:
        url = BASE_URL + 'friends/list.json?count=200'
    response = json.loads(oauth_req(url, config.ACCESS_TOKEN, config.ACCESS_TOKEN_SECRET, "GET"))
    if 'errors' in response:
        print response['errors']
        return []
...
def update_friend_list_db():
    ...
    # twitter_friend_list = I_FOLLOW
    for user in twitter_friend_list:
        if dbCommand.get_user_by_id(user['id']) == None:
            dbCommand.add_user(user)
        # else:
        dbCommand.update_following_me(user['id_str'], False)

def update_following_me_db():
    twitter_follower_list = get_twitter_follower_list([],1)
    # twitter_follower_list = FOLLOW_ME
    for id in twitter_follower_list:
        if dbCommand.get_user_by_id(id) == None:
            dbCommand.add_user_id(id)
        else:
            dbCommand.update_following_me(id, True)
